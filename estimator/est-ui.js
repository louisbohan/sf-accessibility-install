/**
 * est-ui.js — Client-side wizard UI for the Accessibility Install Estimator
 *
 * Three-step flow:
 *   1. Pick a service from a grid
 *   2. Answer ≤4 questions (drives modifiers)
 *   3. Enter name/phone/ZIP to reveal the range (lead magnet gate)
 *
 * The estimator NEVER shows a single fixed price — always a {low, high} range.
 */
...
const QUESTIONS = {
  ...
  vpl: [
    ...
    { key: 'concrete_pad', label: 'Concrete landing/pad already in place?', type: 'bool', invert: true }, // invert: true means "yes" adds cost
...
// UI State
// ---------------------------------------------------------------------------
let pricing = null;          // Loaded from pricing.json
let state = {
  serviceId: null,
  qty: 1,
  rise_in: null,
  ...
  container.querySelectorAll('input').forEach((input) => {
    input.addEventListener('change', () => {
      ...
      if (q.type === 'bool') {
        ...
        if (key === 'qty') {
          state.qty = isNaN(val) ? 1 : val;
        } else if (key === 'rise_in') {
          state.rise_in = isNaN(val) ? null : val;
        } else {
          // Generic qty modifier (e.g., each_turn)
          state.answers[key] = isNaN(val) ? 0 : val;
        }
      }
    });
  });
...
function initLeadForm() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    // Remove qty/rise_in from answers so they don't double-count as modifiers
    ...
    try {
      // Try server-side estimate first (hides wholesale)
      ...
      if (res.ok) {
        ...
        // Fallback: compute client-side (wholesale visible in devtools — week 1 mode)
        r = estimateBundle(pricing, [sel]);
      }
      ...
      // Show permit warning if any item likely needs permit
      const permitEl = $('#est-permit');
      if (permitEl) {
        permitEl.hidden = !r.items?.some((i) => i.permitLikely);
      }

      show(4);

      // Fire lead capture webhook (OpenClaw / Web3Forms / etc.)
...
async function fireLeadWebhook(payload) {
  try {
    // Replace with your actual intake endpoint
    ...
    // Non-blocking: intake failure shouldn't break UX
...
function initNavigation() {
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => show(3));
  }

  // Back buttons (if present in markup)
...
// Bootstrap
// ---------------------------------------------------------------------------
export async function initEstimator(pricingUrl = './pricing.json') {
  try {
    const res = await fetch(pricingUrl);
    pricing = await res.json();
  } catch (e) {
    console.error('Failed to load pricing:', e);
    // Graceful degradation: show error in step 1
    const grid = $('#est-services');
    if (grid) grid.innerHTML = '<p class="est-error">Unable to load pricing. Please refresh or call us.</p>';
    return;
  }
